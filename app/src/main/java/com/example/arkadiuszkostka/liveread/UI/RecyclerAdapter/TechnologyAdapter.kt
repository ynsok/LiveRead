...
class TechnologyAdapter(private var listner: OnItemPosition): RecyclerView.Adapter<TechnologyAdapter.ViewHolder>() {
    var data: List<TechnologyEntry>by Delegates.observable(emptyList())
    { property, oldValue, newValue -> update(oldValue, newValue) { old, new -> old.id == new.id } }


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val dataBindingUtil: ViewDataBinding = DataBindingUtil.inflate(
                parent.inflate()
                , R.layout.list_item_technology, parent
                , false)
